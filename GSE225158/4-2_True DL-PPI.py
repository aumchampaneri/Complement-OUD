...
def load_inflammatory_pathways(pathway_file="/Users/aumchampaneri/PycharmProjects/Complement-OUD/GSE225158/KEGG outputs/kegg_inflammatory_pathways.csv"):
...
def create_complement_training_data(output_dir="DL-PPI outputs"):
    ...
    try:
        ...
        # Download PPI data
        request_url = f"{base_url}/{output_format}/{method}?identifiers=C3,C5,CFB,CFH,CFD&species=9606&add_nodes=20&network_type=physical"
        response = requests.get(request_url)

        # Parse response
        ppi_df = pd.read_csv(StringIO(response.text), sep='\t', header=None)
        ...
        neg_data = [(p1, p2, 0) for p1, p2 in neg_pairs]
        ...
        print(f"Created {len(train_df)} training pairs, {len(pos_data)} positive, {len(neg_data)} negative")
        ...
        for protein_id in tqdm(proteins, desc="Downloading protein sequences"):
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"Could not retrieve sequence for {protein_id}")
            except Exception as e:
...
def train_ppi_model(train_pairs, train_labels, protein_sequences_dict,
    ...
    class PPIDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            # Get protein sequences
            seq1 = self.sequences_dict.get(p1, "")
            seq2 = self.sequences_dict.get(p2, "")
