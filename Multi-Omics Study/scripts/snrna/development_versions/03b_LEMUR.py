...
OUTPUT_DIR = f"{BASE_DIR}/results/snrna_scvi/lemur_regional_analysis"
...
ANALYSIS_CONFIG = {
    ...
    # Analysis contrasts
    'include_sex_analysis': True, # Include sex interaction analysis
    'include_regional_analysis': True, # Include regional contrast analysis (Caudate vs Putamen)
}
...
def load_and_prepare_data():
    ...
    if 'Sex' in adata.obs.columns and 'sex' not in adata.obs.columns:
        adata.obs['sex'] = adata.obs['Sex']

    # Standardize region column
    if 'Region' in adata.obs.columns and 'region' not in adata.obs.columns:
    ...
    # Display sex by condition cross-tabulation
    if 'sex' in adata.obs.columns:
        print("   Sex by Condition distribution:")
        crosstab = adata.obs.groupby(['sex', 'condition']).size().unstack(fill_value=0)
        for sex in crosstab.index:
            for condition in crosstab.columns:
                count = crosstab.loc[sex, condition]
                print(f"     {sex} {condition}: {count:,}")
...
def run_pylemur_analysis(adata, n_embedding=None, max_cells=None):
    """Run comprehensive pyLEMUR analysis including sex interactions"""
    print("\n🌊 RUNNING COMPREHENSIVE LEMUR ANALYSIS")
    print("=" * 50)
    ...
    try:
        ...
        if adata.n_obs > max_cells:
            ...
            adata_sample = subsample_balanced_with_sex(adata, max_cells)
        else:
        ...
        # Run multiple LEMUR analyses
        results = {}

        # 1. Main condition effect (OUD vs Control)
        print("\n   📊 Analysis 1: Main Condition Effect (OUD vs Control)")
        results['condition'] = run_single_lemur_model(
            X_norm, adata_hvg.obs, gene_names, "~ condition", n_embedding, "condition"
        )

        # 2. Sex interaction model if sex is available
        if ANALYSIS_CONFIG['include_sex_analysis'] and 'sex' in adata_hvg.obs.columns:
            print("\n   📊 Analysis 2: Sex + Condition Interaction")
            results['sex_interaction'] = run_single_lemur_model(
                X_norm, adata_hvg.obs, gene_names, "~ sex + condition + sex:condition", n_embedding, "interaction"
            )

            # 3. Male-specific analysis
            print("\n   📊 Analysis 3: Male-specific (OUD vs Control)")
            male_mask = adata_hvg.obs['sex'] == 'M'
            if male_mask.sum() > 100:  # Ensure sufficient cells
                results['male_specific'] = run_single_lemur_model(
                    X_norm[male_mask], adata_hvg.obs[male_mask], gene_names, "~ condition", n_embedding, "condition"
                )

            # 4. Female-specific analysis
            print("\n   📊 Analysis 4: Female-specific (OUD vs Control)")
            female_mask = adata_hvg.obs['sex'] == 'F'
            if female_mask.sum() > 100:  # Ensure sufficient cells
                results['female_specific'] = run_single_lemur_model(
                    X_norm[female_mask], adata_hvg.obs[female_mask], gene_names, "~ condition", n_embedding, "condition"
                )

        # 5. Regional analysis if region information is available
        if ANALYSIS_CONFIG.get('include_regional_analysis', True) and 'region' in adata_hvg.obs.columns:
            print("\n   🧠 Regional Analysis: Adding Tier 2 Contrasts")
            
            # 5A. Caudate-specific analysis (OUD vs Control)
            print("   📊 Analysis 5A: Caudate-specific (OUD vs Control)")
            caudate_mask = adata_hvg.obs['region'] == 'Caudate'
            if caudate_mask.sum() > 100:  # Ensure sufficient cells
                results['caudate_specific'] = run_single_lemur_model(
                    X_norm[caudate_mask], adata_hvg.obs[caudate_mask], gene_names, "~ condition", n_embedding, "condition"
                )
                print(f"     Caudate cells: {caudate_mask.sum()}")
            else:
                print(f"     ⚠️ Insufficient Caudate cells: {caudate_mask.sum()}")

            # 5B. Putamen-specific analysis (OUD vs Control)
            print("   📊 Analysis 5B: Putamen-specific (OUD vs Control)")
            putamen_mask = adata_hvg.obs['region'] == 'Putamen'
            if putamen_mask.sum() > 100:  # Ensure sufficient cells
                results['putamen_specific'] = run_single_lemur_model(
                    X_norm[putamen_mask], adata_hvg.obs[putamen_mask], gene_names, "~ condition", n_embedding, "condition"
                )
                print(f"     Putamen cells: {putamen_mask.sum()}")
            else:
                print(f"     ⚠️ Insufficient Putamen cells: {putamen_mask.sum()}")

            # Display regional distribution
            if 'region' in adata_hvg.obs.columns:
                print("   Regional distribution:")
                region_counts = adata_hvg.obs['region'].value_counts()
                for region, count in region_counts.items():
                    oud_count = adata_hvg.obs[(adata_hvg.obs['region'] == region) & (adata_hvg.obs['condition'] == 'OUD')].shape[0]
                    ctrl_count = adata_hvg.obs[(adata_hvg.obs['region'] == region) & (adata_hvg.obs['condition'] == 'Control')].shape[0]
                    print(f"     {region}: {count:,} cells (OUD: {oud_count:,}, Control: {ctrl_count:,})")

        # Combine results
        print(f"\n   ✅ Comprehensive analysis completed!")
        print(f"   Analyses performed: {list(results.keys())}")
        
        # Summary of analyses performed
        analysis_summary = []
        if 'condition' in results:
            analysis_summary.append("✅ Main condition effect (OUD vs Control)")
        if 'sex_interaction' in results:
            analysis_summary.append("✅ Sex + condition interaction")
        if 'male_specific' in results:
            analysis_summary.append("✅ Male-specific analysis")
        if 'female_specific' in results:
            analysis_summary.append("✅ Female-specific analysis")
        if 'caudate_specific' in results:
            analysis_summary.append("✅ Caudate-specific analysis")
        if 'putamen_specific' in results:
            analysis_summary.append("✅ Putamen-specific analysis")
            
        print("   📊 Analysis Summary:")
        for summary in analysis_summary:
            print(f"     {summary}")

        # Use the main condition effect as primary result for backward compatibility
        main_result = results['condition']
        main_result.update({
            'all_results': results,
            'adata_used': adata_sample,
            'method': 'pylemur_comprehensive'
        })
...
def export_lemur_results(results, adata):
    ...
    # Export regional results if available
    if 'all_results' in results and results['all_results']:
        print(f"   📊 Exporting stratified analysis results...")
        export_stratified_results(results['all_results'], adata)
...
def export_stratified_results(all_results, adata):
    """Export results from stratified analyses (sex, regional)"""
    print(f"   🧠 Exporting stratified analysis results...")
    
    # Create stratified results directory
    stratified_dir = f"{TABLES_DIR}/stratified_analyses"
    ...
    for analysis_name, result in all_results.items():
        if result is None or 'differential_results' not in result:
            continue
            
        de_df = result['differential_results']
        n_sig = de_df['significant'].sum() if 'significant' in de_df.columns else 0
        n_up = ((de_df['significant']) & (de_df['lfc'] > 0)).sum() if 'significant' in de_df.columns else 0
        n_down = ((de_df['significant']) & (de_df['lfc'] < 0)).sum() if 'significant' in de_df.columns else 0
        
        # Export individual analysis results
        analysis_filename = f"{stratified_dir}/lemur_{analysis_name}_differential_expression.csv"
        de_df.to_csv(analysis_filename, index=False)
        print(f"     {analysis_name}: {analysis_filename}")
        
        # Export significant genes only
        if n_sig > 0:
            sig_genes = de_df[de_df['significant']].copy().sort_values('adj_pval')
            sig_filename = f"{stratified_dir}/lemur_{analysis_name}_significant_genes.csv"
            sig_genes.to_csv(sig_filename, index=False)
            print(f"     {analysis_name} significant: {sig_filename}")
        
        # Add to summary
        summary_data.append({
            'Analysis': analysis_name,
            'Description': get_analysis_description(analysis_name),
            'Total_Genes': len(de_df),
            'Significant_Genes': n_sig,
            'Percent_Significant': n_sig / len(de_df) * 100 if len(de_df) > 0 else 0,
            'Upregulated': n_up,
            'Downregulated': n_down,
            'Mean_Abs_LFC': de_df['lfc'].abs().mean() if len(de_df) > 0 else 0,
            'Max_Abs_LFC': de_df['lfc'].abs().max() if len(de_df) > 0 else 0
        })
    
    # Export comprehensive summary
    if summary_data:
        summary_df = pd.DataFrame(summary_data)
        comprehensive_summary_filename = f"{stratified_dir}/comprehensive_stratified_summary.csv"
        summary_df.to_csv(comprehensive_summary_filename, index=False)
        print(f"   📊 Comprehensive summary: {comprehensive_summary_filename}")
        
        # Print summary table
        print("\n   📋 STRATIFIED ANALYSIS SUMMARY:")
        for _, row in summary_df.iterrows():
            print(f"     {row['Analysis']}: {row['Significant_Genes']} genes ({row['Percent_Significant']:.1f}%)")

def get_analysis_description(analysis_name):
    """Get human-readable description for analysis"""
    descriptions = {
        'condition': 'Main effect: OUD vs Control (All cells)',
        'sex_interaction': 'Sex + Condition interaction',
        'male_specific': 'OUD vs Control (Males only)',
        'female_specific': 'OUD vs Control (Females only)',
        'caudate_specific': 'OUD vs Control (Caudate only)',
        'putamen_specific': 'OUD vs Control (Putamen only)'
    }
    return descriptions.get(analysis_name, analysis_name.replace('_', ' ').title())
