...
if(!is.null(gsea_data) && nrow(df_filtered) > 0) {
  tryCatch({
    ...
    if(length(matched_idx) > 0) {
      ...
      png(enrich_dotplot_file, width = 2000, height = 2800, res = 300)
...
if(!is.null(gsea_data) && nrow(df_filtered) > 0) {
  tryCatch({
    ...
    if(length(top_pathway_ids) > 0) {
      ...
      png(overlaid_plot_file, width = 5000, height = 3000, res = 300)
      ...
      for (i in seq_along(top_pathway_ids)) {
        ...
        png(gseaplot_file, width = 4500, height = 3000, res = 300)
...
if(nrow(df_filtered) > 0) {
  tryCatch({
    ...
    # Create a copy of the dataframe for plotting with modified descriptions
    plot_df <- df_filtered %>%
      mutate(
        # Replace underscores with spaces and wrap text
        Description_Wrapped = str_replace_all(Description, "_", " "),
        Description_Wrapped = str_wrap(Description_Wrapped, width = 40)
      )

    p <- ggplot(plot_df, aes(x = reorder(Description_Wrapped, NES), y = NES,
                         fill = NES > 0)) +
      geom_col(width = 0.7) +
...
if(nrow(df_filtered) > 0 && "core_enrichment" %in% colnames(df_filtered) && !is.null(res)) {
  tryCatch({
    ...
    if(all(lengths(genes_list) == 0)) {
      ...
      png(heatmap_file, width = 3600, height = 5100, res = 300)
      ...
      if(!is.null(gsea_data)) {
        ...
        png(concept_net_file, width = 4200, height = 3000, res = 300)
      ...
      if(!is.null(gsea_data)) {
        ...
        png(ridge_file, width = 3000, height = 2400, res = 300)
...
if(!is.null(gsea_data) && nrow(df_filtered) > 0) {
  tryCatch({
    ...
    if(length(matched_idx) > 0) {
      ...
      png(network_plot_file, width = 4500, height = 3000, res = 300)
