...
pdf("cellchat_networks.pdf", width=12, height=8)
par(mfrow = c(1,2))
...
write.csv(top_pathways, "top_signaling_pathways.csv")

# Plot specific pathways
pdf("cellchat_top_pathways.pdf", width=10, height=8)
...
pdf("cellchat_pathway_circles.pdf", width=12, height=10)
for (i in 1:length(pathways.show)) {
  netVisual_aggregate(cellchat, signaling = pathways.show[i], layout = "circle")
...
pdf("cellchat_roles.pdf", width=10, height=8)
